...
def crop_image_with_faces(image_path,aspect_ratio,output_dir,iqty,landscape,server,client):
    print(image_path)
    imgread = cv2.imread(str(image_path))
    ...
    image_height, image_width, _ = imgread.shape 

    if image_width > image_height:
        imgread = cv2.rotate(imgread,cv2.ROTATE_90_CLOCKWISE)
    ...
    if len(faces) > 0:
        ...
        crop_image_with_aspect_ratio(image_path=image_path,aspect_ratio=aspect_ratio,output_dir=output_dir,iqty=iqty,landscape=landscape,server=server,client=client)


def crop_image_with_aspect_ratio(image_path,aspect_ratio,output_dir,iqty,landscape,server,client):
...
class CropBeauty:
    def __init__(self,images,aspect_ratio,output_dir,facemode=False,landscape=False,server='',client=''):
        self.images = images 
    ...
    def crop(self):
        ...
        for i in self.images:
            if self.facemode:
                crop_image_with_faces(image_path=i[0],aspect_ratio=aspect_ratio,output_dir=self.output_di,iqty=int(i[1]),landscape=self.landscape,server=self.server,client=self.client)
            else:
                crop_image_with_aspect_ratio(image_path=i[0],aspect_ratio=aspect_ratio,output_dir=self.output_di,iqty=int(i[1]),landscape=self.landscape,server=self.server,client=self.client)
